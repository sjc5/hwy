...
async function generate_public_file_map() {
  ...
  const map_file_path = path.resolve('./dist/public-map.json')
  const reverse_map_file_path = path.resolve('./dist/public-reverse-map.json')

  await Promise.all([
    fs.promises.writeFile(
      map_file_path,
      JSON.stringify(file_map, null, 2),
      'utf-8'
    ),
    fs.promises.writeFile(
      reverse_map_file_path,
      JSON.stringify(reverse_file_map, null, 2),
      'utf-8'
    ),
  ])
}
