...
import (
	...
	"github.com/sjc5/river/x/kit/headblocks"
...
func (h *Hwy[C]) GetUIHandler(nestedRouter *mux.NestedRouter, coreDataTask *CoreDataTask[C]) http.Handler {
...
func GetIsJSONRequest(r *http.Request) bool {
	return r.URL.Query().Get("hwy_json") == "1"
}

func (h *Hwy[C]) GetActionsHandler(router *mux.Router) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		res := response.New(w)
		res.SetHeader("X-Hwy-Build-Id", h._buildID)
